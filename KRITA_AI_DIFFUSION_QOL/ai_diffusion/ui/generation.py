...
from .widget import (
    WorkspaceSelectWidget, StyleSelectWidget, StrengthWidget, QueueButton,
    GenerateButton, ErrorBox, create_wide_tool_button,
    # ADDED (import PromptHistoryButton)
    PromptHistoryButton
)
...
class GenerationWidget(QWidget):
    ...
    def __init__(self):
        ...
        # Region prompt widget
        self.region_prompt = RegionPromptWidget(self)
        
        # 1) CREATE NEW BUTTONS:
        self.upscale_now_button = QPushButton(_("Upscale"), self)
        self.upscale_now_button.setToolTip(_("Immediately upscale the image using current Upscale settings"))
        self.upscale_now_button.clicked.connect(self._do_instant_upscale)

        self.undo_upscale_button = QPushButton(_("Undo Upscale"), self)
        self.undo_upscale_button.setToolTip(_("Undo the last upscale operation"))
        self.undo_upscale_button.clicked.connect(self._undo_upscale)

        # 2) Add them in a row above the history button:
        upscale_layout = QHBoxLayout()
        upscale_layout.addWidget(self.upscale_now_button)
        upscale_layout.addWidget(self.undo_upscale_button)
        layout.addLayout(upscale_layout)
        
        # ADDED: Place region_prompt + prompt history button side by side
        region_prompt_layout = QHBoxLayout()
        region_prompt_layout.addWidget(self.region_prompt)

        # ADDED: Our new prompt-history button, which shows a dropdown with normal/favorites
        self.prompt_history_button = PromptHistoryButton(self._model, self)
        region_prompt_layout.addWidget(self.prompt_history_button)

        layout.addLayout(region_prompt_layout)

        # Strength, "Add Region," "Add Control" row
    ...
    def model(self, model: Model):
        if self._model != model:
            ...
            self._model_bindings = [
                ...
                self.generate_button.clicked.connect(self._store_prompt_in_history),
            ]
    ...
    def _store_prompt_in_history(self):
        # 1) Pull the user's current positive text from your model or prompt widget
        #    Typically, you might do something like:
        text = self.model.regions.active_or_root.positive or ""
        # 2) Ensure we actually have text
        if not text.strip():
            return  # Don’t store empty prompts

        # 3) Optionally guess a default name for new prompts
        #    You could do something like a timestamp or just leave it blank:
        name = ""   # or something like f"Prompt @ {datetime.now():%H:%M:%S}"

        # 4) Add to the normal prompt history in the manager
        self.prompt_history_button.manager.add_prompt(name, text)
    def _do_instant_upscale(self):
        """
        Immediately upscale the image using the current Upscale settings 
        from the plugin's Upscale tab or model.
        """
        # The simplest approach is to call the same routine that the UpscaleWidget calls:
        # (Inside upscale.py, there's a method self.model.upscale_image()) 
        self.model.upscale_image()

    def _undo_upscale(self):
        """
        Undo the last upscale step in Krita. 
        This calls Krita's own undo system, so it only works if your plugin 
        performed an actual undoable operation (e.g. painting on a layer).
        """
        doc = Krita.instance().activeDocument()
        # Possibly do multiple undos
        for i in range(4):
